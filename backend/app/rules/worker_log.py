...
def record_if_changed(camera_id: str, prev_status: WorkerStatus | None, new_status: WorkerStatus, now_iso: str) -> None:
    """직전 상태와 event가 달라졌을 때만 로그 한 줄을 남긴다."""
    if prev_status is not None and prev_status.event == new_status.event:
        return
    _append(
        camera_id,
        WorkerEventLogEntry(track_id=new_status.track_id, event=new_status.event, zone=new_status.last_zone, at=now_iso),
    )
