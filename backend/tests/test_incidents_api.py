...
from app.models.schemas import AlarmSource, FireAlert, WorkerEvent, WorkerStatus
...
def test_timeline_merges_and_sorts_all_sources(monkeypatch, tmp_path):
    ...
    assert sources == {"fire_alert", "worker", "ppe_violation", "zone_situation"}
    # fire_alert 1 + worker 2 + ppe_violation 1 + zone_situation 1
    assert len(rows) == 5
